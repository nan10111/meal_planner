...
const theme = {
  ...
  card: "#1e2230",
  accent: "#e8883a",
  accentSoft: "rgba(232,136,58,0.15)",
  accentGlow: "rgba(232,136,58,0.3)",
  text: "#e8e6e1",
  textDim: "#8a8b94",
  textMuted: "#5a5b64",
  border: "#2a2d3a",
  danger: "#e85454",
  dangerSoft: "rgba(232,84,84,0.15)",
  success: "#4ade80",
  successSoft: "rgba(74,222,128,0.12)",
  tag: "#6366f1",
  tagSoft: "rgba(99,102,241,0.15)",
  radius: "14px",
  radiusSm: "10px",
...
export default function MealPlannerApp() {
  ...
  useEffect(() => {
    (async () => {
      const r = await dbGetAll("recipes");
      const w = await dbGetAll("weekplan");
      setRecipes(r);
      const plan = {};
      w.forEach(item => { plan[item.id] = item; });
      setWeekPlan(plan);
      setLoaded(true);
    })();
  }, []);
  ...
  const saveRecipe = async (recipe) => {
    ...
    setRecipes(prev => {
      ...
      if (idx >= 0) { const n = [...prev]; n[idx] = recipe; return n; }
      return [...prev, recipe];
    });
    setShowForm(false);
    setEditRecipe(null);
  ...
  const assignMeal = async (day, meal, recipeId) => {
    ...
    await dbPut("weekplan", item);
    setWeekPlan(prev => ({ ...prev, [key]: item }));
  };

  const clearMeal = async (day, meal) => {
    const key = `${day}-${meal}`;
    await dbDelete("weekplan", key);
  ...
  const generatePlan = async () => {
    ...
    for (const day of DAYS) {
      for (const meal of MEALS) {
        ...
        const item = { id: key, day, meal, recipeId: r.id };
        await dbPut("weekplan", item);
        newPlan[key] = item;
      }
    }
    ...
    <div style={{ background: theme.bg, minHeight: "100vh", fontFamily: theme.font, color: theme.text, paddingBottom: 90, position: "relative" }}>
      ...
      <div style={{ padding: "16px 16px 0" }}>
        ...
        {view === "recipes" && showForm && (
          <RecipeForm recipe={editRecipe} onSave={saveRecipe}
