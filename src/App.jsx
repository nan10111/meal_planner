...
function parseIngredientString(str) {
  const unitMap = {
    ...
    "msp": "Prise", "große": "Stück", "kleine": "Stück",
    "einige": "Stück", "½": "Stück", "¼": "Stück",
  };

  const clean = str.replace(/^[-•●◦▪·]\s*/, "").trim();
  if (!clean) return null;

  // Match: optional fraction/number, optional unit, then name
  const match = clean.match(/^([\d½¼¾⅓⅔.,\/\s]+)?\s*(g|gr|gramm|kg|kilo|ml|milliliter|l|liter|el|esslöffel|essl|tl|teelöffel|teel|stk|stück|stck|st|prise|prisen|bund|dose|dosen|becher|scheibe|scheiben|pkg|pck|packung|päckchen|tasse|tassen|zehe|zehen|msp|große|kleine)?\s*(.+)/i);

  if (!match || !match[3]) return { name: clean, amount: 1, unit: "Stück" };

  let amount = 1;
  if (match[1]) {
    let raw = match[1].replace(",", ".").trim();
    raw = raw.replace("½", "0.5").replace("¼", "0.25").replace("¾", "0.75").replace("⅓", "0.33").replace("⅔", "0.67");
    if (raw.includes("/")) {
  ...
  const unit = unitMap[unitRaw] || (match[2] ? "Stück" : "Stück");
...
function RecipeImport({ onSave, onCancel }) {
  ...
  const handleUrlImport = async () => {
    ...
    setError(null);
    try {
      const resp = await fetch(`/api/recipe?url=${encodeURIComponent(url.trim())}`);
      const data = await resp.json();
      if (!resp.ok) throw new Error(data.error || "Fehler beim Abrufen");

      const ingredients = (data.ingredients || []).map(str => parseIngredientString(str)).filter(Boolean);

      setEditName(data.name || "");
      setEditServings(data.servings || 4);
      setEditIngredients(ingredients.length > 0 ? ingredients : [{ name: "", amount: "", unit: "g" }]);
      setEditSteps(data.steps || "");
      setEditPhoto(data.image || null);
      setParsed(true);
    } catch (e) {
      setError(e.message || "Import fehlgeschlagen. Versuche den Text-Import.");
    }
    setLoading(false);
  };
  ...
  const handleTextImport = () => {
    ...
    for (const line of lines) {
      ...
      if (line.match(/^(zutaten|zutat|ingredients):?\s*$/i)) { inSteps = false; continue; }
      if (line.match(/^(zubereitung|anleitung|preparation|schritte|steps|so geht'?s|und so wird'?s gemacht):?\s*$/i)) { inSteps = true; continue; }

      // Servings
      const servMatch = line.match(servingsRegex);
      if (servMatch) { servings = parseInt(servMatch[1]) || 4; continue; }

      // Name: first line that doesn't look like an ingredient
      if (!name && !line.match(/^\d/) && !line.match(servingsRegex)) {
        if (!line.match(/^(zutaten|zubereitung)/i)) { name = line; continue; }
      }

      if (inSteps) {
        steps.push(line.replace(/^(\d+[\.\)]\s*|schritt\s*\d+:?\s*)/i, ""));
        continue;
      }

      // Try to parse as ingredient
      const ing = parseIngredientString(line);
      if (ing && ing.name.length > 1 && ing.name.length < 80) {
        ingredients.push(ing);
      } else if (ingredients.length > 0) {
        // After first ingredients found, remaining text = steps
        inSteps = true;
        steps.push(line.replace(/^(\d+[\.\)]\s*|schritt\s*\d+:?\s*)/i, ""));
      }
    }

    setEditName(name || "Importiertes Rezept");
  ...
  const handleSave = () => {
    ...
    const validIngs = editIngredients.filter(i => i.name && i.name.trim());
    onSave({
      id: `recipe_${Date.now()}`,
      name: editName.trim(),
  ...
  return (
    <div>
      ...
      {!parsed ? (
        <>
          {/* Mode toggle */}
          <div style={{ display: "flex", gap: 0, marginBottom: 16, borderRadius: theme.radiusSm, overflow: "hidden", border: `1px solid ${theme.border}` }}>
            {[{ id: "url", label: "🔗 URL eingeben" }, { id: "text", label: "📝 Text einfügen" }].map(m => (
              <button key={m.id} onClick={() => { setMode(m.id); setError(null); }}
                style={{
                  ...
                  fontFamily: theme.font, transition: "all .2s",
                  background: mode === m.id ? theme.accent : theme.surface,
                  color: mode === m.id ? "#fff" : theme.textDim,
                }}>{m.label}</button>
            ))}
          </div>
          ...
          {mode === "url" ? (
            <>
              <p style={{ fontSize: 13, color: theme.textDim, marginBottom: 12, lineHeight: 1.5 }}>
                Funktioniert mit Chefkoch, Lecker.de, REWE, EmmakochEinfach und vielen weiteren Rezeptseiten.
              </p>
              <input value={url} onChange={e => setUrl(e.target.value)} placeholder="https://www.chefkoch.de/rezepte/..."
                onKeyDown={e => { if (e.key === "Enter") handleUrlImport(); }}
                style={{ ...inputStyle, marginBottom: 12 }} />
              <Btn onClick={handleUrlImport} disabled={loading}
                style={{ width: "100%", justifyContent: "center", padding: "14px 20px", fontSize: 16, opacity: loading ? 0.6 : 1 }}>
                {loading ? "⏳ Wird geladen..." : "🔍 Rezept abrufen"}
              </Btn>
            </>
          ) : (
            <>
              <p style={{ fontSize: 13, color: theme.textDim, marginBottom: 12, lineHeight: 1.5 }}>
                Kopiere ein Rezept und füge es hier ein. Achte darauf, dass Zutaten auf einzelnen Zeilen stehen.
              </p>
              <textarea value={text} onChange={e => setText(e.target.value)} rows={12}
                placeholder={"Spaghetti Bolognese\n\nFür 4 Portionen\n\n500 g Spaghetti\n400 g Hackfleisch\n2 Dose Tomaten\n1 Zwiebel\n2 EL Olivenöl\n\nZwiebel hacken und anbraten...\nHackfleisch dazu geben..."}
                style={{ ...inputStyle, resize: "vertical", marginBottom: 12, fontFamily: "monospace", fontSize: 13, lineHeight: 1.5 }} />
              <Btn onClick={handleTextImport}
          ...
          {error && (
            <div style={{ background: theme.dangerSoft, border: `1px solid ${theme.danger}`, borderRadius: theme.radiusSm, padding: "10px 14px", marginTop: 12, fontSize: 13 }}>
              ❌ {error}
            </div>
          )}
        </>
        ...
        <>
          <div style={{ background: theme.successSoft, border: `1px solid ${theme.success}`, borderRadius: theme.radiusSm, padding: "10px 14px", marginBottom: 16, fontSize: 13 }}>
            ✅ {editIngredients.filter(i => i.name).length} Zutaten erkannt
          ...
          {editPhoto && (
            <div style={{
              width: "100%", height: 160, borderRadius: theme.radius, marginBottom: 16,
              backgroundImage: `url(${editPhoto})`, backgroundSize: "cover", backgroundPosition: "center"
            }} />
          )}
          ...
          <textarea value={editSteps} onChange={e => setEditSteps(e.target.value)} rows={6}
            style={{ ...inputStyle, resize: "vertical", marginBottom: 12 }} />

          <div style={{ display: "flex", gap: 10 }}>
            <Btn variant="secondary" onClick={() => { setParsed(null); setError(null); }} style={{ flex: 1, justifyContent: "center" }}>← Zurück</Btn>
